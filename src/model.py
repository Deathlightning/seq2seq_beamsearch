...
class EncoderRNN(nn.Module):
    def __init__(self, embd_size, enc_h_size, dec_h_size, v_size):
    ...
    def forward(self, x):
        ...
        # 把双向GRU的两层hidden拼到一起再映射，激活
        # (bs, dec_h_size)
        hidden = torch.tanh(self.f_concat_h(torch.cat((hidden[-2], hidden[-1]), dim=1))) 
...
class DecoderRNN(nn.Module):
    def __init__(self, embd_size, dec_h_size, v_size):
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio=0.5):
        ...
        encoder_outs, hidden = self.encoder(src) # (T, bs, encH*), (bs, H)

        #first input to the decoder is the <sos> tokens
        inp = trg[0, :] # (bs) # first token

        for t in range(1, trg_len):
            output, hidden = self.decoder(inp, hidden) # (bs, V), (bs, H)
